  ...
  for (const fragment of allFragments) {
    if (typeof fragment !== 'string') {
      ...
      while (i < fragment.length) {
        ...
        if (char === '<') {
          if (buffer !== "") {
            yield { type: TokenTypes.RAW_TEXT, value: buffer }
          };
          buffer = "";
...
const commitNode = (tree, node) => {
  const newTree = [ ...tree, node ]
  return [newTree, { props: {} }]
  ...
  for (const token of streamTokens(merged)) {
    switch (token.type) {
      ...
      case TokenTypes.TAG_START: {
        console.log(token, previousToken, currentNode);
      ...
      case TokenTypes.NODE_END: {
        [ tree, currentNode ] = commitNode(tree, currentNode); 
      ...
      case TokenTypes.RAW_TEXT: {
        if (previousToken.type === TokenTypes.TAG_END) {
          const { children } = currentNode;
          const [ tree ] = commitNode(children || [], token.value);
